...
class FeedForwardA(nn.Module):
    def __init__(self, in_dim, out_dim, ffn_expansion_factor, bias):
        ...
        hidden_features = int(self.dim*ffn_expansion_factor)
        self.hid_fea = hidden_features
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        ...
        m.append(nn.AdaptiveAvgPool2d((448, 640)))
...
class SRWABA(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp_ratio = mlp_ratio
        self.norm1 = norm_layer(dim)
        self.norm1_aux = norm_layer(dim)
        self.qkv = nn.Linear(dim, out_dim // 2 * 3, bias=qkv_bias)
        self.qkv_aux = nn.Linear(dim, out_dim // 2 * 3, bias=qkv_bias)
        self.proj = nn.Linear(out_dim, out_dim)
        self.branch_num = 2
        self.get_v = nn.Conv2d(dim // 2, dim, kernel_size=3, stride=1, padding=1,groups=dim//2) # DW Conv
        ...
        self.mlp = Mlp(in_features=out_dim, hidden_features=mlp_hidden_dim, out_features=out_dim, act_layer=act_layer)

    def forward(self, x, x_aux, x_size, params, attn_mask=NotImplementedError):
        h, w = x_size
        self.h,self.w = x_size
        ...
        shortcut = x
        x = self.norm1(x)
        qkv = self.qkv(x).reshape(b, -1, 3, c_qkv).permute(2, 0, 1, 3) # 3, B, HW, C
        v = qkv[2].transpose(-2,-1).contiguous().view(b, c_qkv, h, w)

        x_aux = self.norm1_aux(x_aux)
        qkv_aux = self.qkv_aux(x_aux).reshape(b, -1, 3, c_qkv).permute(2, 0, 1, 3) # 3, B, HW, C
        v_aux = qkv_aux[2].transpose(-2,-1).contiguous().view(b, c_qkv, h, w)

        # cyclic shift
        if self.shift_size[0] > 0 or self.shift_size[1] > 0:
            qkv = qkv.view(3, b, h, w, c_qkv)
            qkv_aux = qkv_aux.view(3, b, h, w, c_qkv)
            # H-Shift
            qkv_0 = torch.roll(qkv[:,:,:,:,:c_qkv//2], shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(2, 3))
            qkv_0 = qkv_0.view(3, b, h*w, c_qkv//2)
            qkv_0_aux = torch.roll(qkv_aux[:,:,:,:,:c_qkv//2], shifts=(-self.shift_size[0], -self.shift_size[1]), dims=(2, 3))
            qkv_0_aux = qkv_0_aux.view(3, b, h*w, c_qkv//2)
            # V-Shift
            qkv_1 = torch.roll(qkv[:,:,:,:,c_qkv//2:], shifts=(-self.shift_size[1], -self.shift_size[0]), dims=(2, 3))
            qkv_1 = qkv_1.view(3, b, h*w, c_qkv//2)
            qkv_1_aux = torch.roll(qkv_aux[:,:,:,:,c_qkv//2:], shifts=(-self.shift_size[1], -self.shift_size[0]), dims=(2, 3))
            qkv_1_aux = qkv_1_aux.view(3, b, h*w, c_qkv//2)

            # H-Rwin
            x1_shift = self.attns[0](torch.cat((qkv_0, qkv_0_aux), dim=-1), h, w, mask=attn_mask[0], rpi=params['rpi_sa_h'], rpe_biases=params['biases_h'])
            # V-Rwin
            x2_shift = self.attns[1](torch.cat((qkv_1, qkv_1_aux), dim=-1), h, w, mask=attn_mask[1], rpi=params['rpi_sa_v'], rpe_biases=params['biases_v'])
            ...
            # H-Rwin
            x1 = self.attns[0](torch.cat((qkv[:,:,:,:c_qkv//2], qkv_aux[:,:,:,:c_qkv//2]), dim=-1), h, w, rpi=params['rpi_sa_h'], rpe_biases=params['biases_h'])
            # V-Rwin
            x2 = self.attns[1](torch.cat((qkv[:,:,:,c_qkv//2:], qkv_aux[:,:,:,c_qkv//2:]), dim=-1), h, w, rpi=params['rpi_sa_v'], rpe_biases=params['biases_v'])
            # Concat
            attened_x = torch.cat([x1,x2], dim=-1)
...
class CRFBA(nn.Module):
    ...
    def forward(self, x, x_aux, x_size, params):
        b, c, h, w = x.shape
        for i in range(self.depth):
            low = x.permute(0, 2, 3, 1)
            low = low.reshape(b, h*w, c)

            low_aux = x_aux.permute(0, 2, 3, 1)
            low_aux = low_aux.reshape(b, h*w, c)

            low = self.srwa_blocks[2*i+1](self.srwa_blocks[2*i](low, low_aux, x_size, params, params['attn_mask']), low_aux, x_size, params, params['attn_mask'])
            low = low.reshape(b, h, w, c)
            low = low.permute(0, 3, 1, 2)
            high = self.hfer_blocks[i](x)
            x = self.hf_blocks[i](low, high)
...
class RCRFGA(nn.Module):
    ...
    def forward(self, x, x_aux, x_size, params):
        self.h, self.w = x_size
        return self.conv(self.residual_group(x, x_aux, x_size, params)) + x
...
class MFG_CRAFT(nn.Module):
    ...
    def __init__(self,
        ...
        self.rgb_in_chans = rgb_in_chans
        self.thermal_in_chans = thermal_in_chans
        self.img_range = img_range
        self.out_chans = out_chans

        self.rgb_mean = torch.Tensor((0.4596, 0.5151, 0.4919)).view(1, 3, 1, 1)# for tisr
        self.thermal_mean = torch.Tensor((0.4383, )).view(1, 1, 1, 1)

        assert upscale in [1,2,4,8,16]
        ...
        embed_dim_each = int(embed_dim/2)
        _act = nn.GELU()
        self.conv_first_rgb = nn.Conv2d(rgb_in_chans, embed_dim, 3, 1, 1)
        # self.conv_first_thermal_large = nn.Conv2d(thermal_in_chans, embed_dim_each, 3, 1, 1)
        self.conv_first_thermal = nn.Conv2d(thermal_in_chans, embed_dim, 3, 1, 1)
        ...
        self.conv_aux_feature_extract = RCRFG(
            dim=embed_dim,
            depth=2,
            num_heads=6,
    ...
    def forward_features(self, x, x_aux):
        ...
        for layer in self.layers:
            x = layer(x, x_aux, x_size, params)

        x = self.norm(x)

        return x
    
    def extract_aux_feature(self, x_aux):
        x_aux_size = (x_aux.shape[2], x_aux.shape[3])

        params = {'attn_mask': (None, None), 'rpi_sa_h': self.relative_position_index_h, 'rpi_sa_v': self.relative_position_index_v, 'biases_v':self.biases_v, 'biases_h':self.biases_h}
        
        x_aux = self.conv_aux_feature_extract(x_aux, x_aux_size, params)

        return x_aux

    def forward(self, x_rgb, x_thermal):
        self.h, self.w = x_rgb.shape[2:]
        self.rgb_mean = self.rgb_mean.type_as(x_rgb)
        x_rgb = (x_rgb - self.rgb_mean) * self.img_range
        self.thermal_mean = self.thermal_mean.type_as(x_rgb)
        x_thermal_ori = x_thermal
        x_thermal = (x_thermal - self.thermal_mean) * self.img_range

        windowsize = 16
        hOri, wOri = x_rgb.size()[-2:]
        mod_pad_h = (windowsize - hOri % windowsize) % windowsize
        mod_pad_w = (windowsize - wOri % windowsize) % windowsize
        x_rgb = F.pad(x_rgb, (0, mod_pad_w, 0, mod_pad_h), 'reflect')
        x_thermal = F.pad(x_thermal, (0, mod_pad_w*self.upscale, 0, mod_pad_h*self.upscale), 'reflect')

        shallow_feature_rgb = self.conv_first_rgb(x_rgb) 
        # shallow_feature_thermal_upsampled = self.conv_first_thermal_large(F.interpolate(x_thermal, size=x_rgb.size()[-2:], mode='bicubic'))
        shellow_feature_input = shallow_feature_rgb

        feature_thermal = self.conv_first_thermal(F.interpolate(x_thermal, size=x_rgb.size()[-2:], mode='bicubic'))
        feature_thermal = self.extract_aux_feature(feature_thermal)

        x = self.conv_after_body(self.forward_features(shellow_feature_input, feature_thermal))#ORI

        if self.upscale > 1:
            x = self.upsample(x)

        x = (x / self.img_range + self.thermal_mean)# + F.interpolate(x_thermal_ori, size=x.size()[-2:], mode='bicubic')
        return x
